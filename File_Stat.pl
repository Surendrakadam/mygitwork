...
my $v_rec_no_min_len  = 0 ;                                     # Record no of minimum lengthﬂ
...
open ( my $Out_FILE , ">" , $p_out_file ) or                    #
...
print $Out_FILE                                                 # Print report header first line
  ...
  "Minimum Fields" . "\t" .                                     # 15
  "Maximum Fields" . "\t" .                                     # 16
  "Optional enclosing character { " .                            # 17
  (
   $p_enc_spr =~ /[[:cntrl:]]/
   ?                                                            # 17
     ord ( $p_enc_spr )
...
sub sGetFieldTypes {                                            # Subroutine to get field types
   ...
   for ( my $idx = 0 ; $idx < $v_max_no_of_fld_rec ; $idx ++ ) {
      ...
      if ( $vft_fld_typ eq "Unknown" ) {
         ...
         and $vft_prv_fld_typ ne "Unicode" ) {
         $h_fld_typ{ $idx } = $vft_fld_typ ;
      } ## end elsif ( $vft_fld_typ eq "Signed Decimal"...)
         ...
         and $vft_prv_fld_typ ne "Unicode" ) {
         $h_fld_typ{ $idx } = $vft_fld_typ ;
      } ## end elsif ( $vft_fld_typ eq "Decimal"...)
         ...
         and $vft_prv_fld_typ ne "Unicode" ) {
         $h_fld_typ{ $idx } = $vft_fld_typ ;
      } ## end elsif ( $vft_fld_typ eq "Signed Integer"...)
         ...
         and $vft_prv_fld_typ ne "Unicode" ) {
         $h_fld_typ{ $idx } = $vft_fld_typ ;
      } ## end elsif ( $vft_fld_typ eq "Integer"...)
...
sub sGetParameter {                                             # Subroutine to get parameter
   ...
   if ( ! $parameters -> getOptions ( $p_GetOptions , 'Usage: FileStts.pl -i infile.txt -f ; -e \" -d -o infile_txt_statistics.csv' ) ) {
      exit ( -1 ) ;                                             # Failure.
   }
...
close $Out_FILE or die "Could not open output file $p_out_file - $!\n" ;
## end sub sGetParameter
#######################################################################
...
=head3 Fields
 ...
 1. first it will determine the datatype of all the fields.
 ...
 7. Count of occurences of invalid character in field as per field number.
 8. No of Occcurence of fields as per field number.
 ...
 12. Record no of maxnimum length of field as per field number. 
 13. Field wise count where optional enclosures used with proper closed.
 14. Field wise count where optional enclosure used only in begining or at end of
 field.
...
=head3 Minimum and Maximum value as per field type
 ...
 operation.
 If datatype is ascii, unicode and unknown gt:greater than lt:less than operator is used.
 If datatype is integer, signed integer ,decimal and signed decimal ( > ,< ) operator  is used.
 Highest precedence of a datatype are as below:
 -Unknown
 -Unicode
 -ascii
 -signed decimal
...
=head2 Warning
 ...
 Field separator contain only one character
 Enclose separator contain only one character
...
=head3 Run parameters
 ...
 encloser_separator       Enclosed separator                   e   $p_enc_spr
...
=head3 Different examples of run parameter to run a procedure:
 ...
 perl File_Stat.pl -i input.txt -f ; -e \" -d  -o input_txt_filestatistics.csv
 
 perl File_Stat.pl -i input.txt -f ; -e \" -o input_txt_filestatistics.csv
 
 perl File_Stat.pl -i input.txt -f ; -o input_txt_filestatistics.csv
 
 perl File_Stat.pl -i input.txt -f ; -e \"
 
 perl File_Stat.pl -i input.txt -f ;
 
 perl File_Stat.pl -i E:\folder1\folder2\input.txt -f ; -e \" -d  -o input_txt_filestatistics.csv
